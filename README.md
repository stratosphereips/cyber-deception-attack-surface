# Cyber Deception Attack Surface Mapping Across the MITRE ATT&CK® Enterprise matrix (v18.1)

An interactive matrix of where **passive infrastructure deception** can be
deployed across the MITRE ATT&CK® Enterprise matrix (v18.1). Every technique
scored on Feasibility, Interaction, Intelligence Yield, and Malice Fidelity,
with a written rationale per entry.

Companion to the paper:

    Valeros, V., Catania, C., Lisý, V., Griffioen, H. (2026) "Decoys Cannot Go Everywhere: Mapping the Deception Surface in MITRE ATT&CK".

**Headline finding:** 80 of 250 technique/tactic entries (~32%) are feasible deception surfaces.

## Dataset in Zenodo

Valeros, V. (2026). CTU-HONEYEVAL-26: Infrastructure Deception Codebook, Decision Rules, and Per-Technique Assessments for MITRE ATT&CK v18.1 (Version 1.3.0) [Dataset]. Zenodo. https://doi.org/10.5281/zenodo.22030181

## License

- Evaluation data: [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)
- ATT&CK fields, technique names, technique IDs, tactic names: © The MITRE Corporation.

----
This is an independent research project, not affiliated with, sponsored by, or endorsed
by The MITRE Corporation. MITRE ATT&CK® and ATT&CK® are registered
trademarks of The MITRE Corporation.
